...
for duration in range(4, max_duration+1, 1):    #detecting typical sequences with different duration
    for idx in range(len(primitive_compact)-duration):
        ...
        """        
        continue_flag = 0
        sequence_extend = sequence + sequence
        for key in key_restore:
            if len(sequence) % len(key) == 0:
                if sum([key == sequence_extend[i:i+len(key)] for i in range(0,len(sequence))]) >= len(sequence) // len(key):
                    continue_flag = 1
                    break
        """
        if continue_flag:
...
with open('C:\\Users\\lenovo\\Desktop\\AI-Project-Portfolio\\Amendments\\scipts for graph construction\\typical_sequences.txt', 'wb') as f:
