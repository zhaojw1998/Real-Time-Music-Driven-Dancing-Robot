...
def set_angel(motion, frame, signal):
    i = frame
    #for left shoulder
    z = np.array(motion[str(i)]['8']) - np.array(motion[str(i)]['0'])# 8 7
    x = np.cross(np.array(motion[str(i)]['11'])-np.array(motion[str(i)]['8']), z) #11 8
    y = np.cross(z, x)
    ...
    shoulder_vector = np.dot(rotation_matrix(np.array([1, 0, 0]), LSR), np.array([1, 0, 0]))
    shoulder_vector = np.dot(rotation_matrix(np.array([0, 1, 0]), LSP), shoulder_vector)
    ...
    #for right shoulder
    z = np.array(motion[str(i)]['8']) - np.array(motion[str(i)]['0']) # 8 7
    x = np.cross(np.array(motion[str(i)]['8'])-np.array(motion[str(i)]['14']), z) #8 14
    y = np.cross(z, x)
    ...
    shoulder_vector = np.dot(rotation_matrix(np.array([1, 0, 0]), RSR), np.array([1, 0, 0]))
    shoulder_vector = np.dot(rotation_matrix(np.array([0, 1, 0]), RSP), shoulder_vector)
    ...
    if signal >0:
        ...
        return (RSP, -RSR, -REY, -RER, LSP, -LSR, -LEY, -LER, -RHR, RHP, RKP, -LHR, LHP, LKP, -RAR, -LAR, RAP, LAP, RHYP, LHYP)
